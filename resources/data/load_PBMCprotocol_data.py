...
def load_PBMC_protocols_data(data_dir, exp=None, protocol=None, protocol_type=None, curate=False):
    ...
    if curate:
        ...
        metadata_df["CellType"].replace(["CD14+ monocyte"], 'Monocytes', inplace=True)
        metadata_df["CellType"].replace(["CD4+ T cell"], 'T cells', inplace=True)
        metadata_df["CellType"].replace(["Cytotoxic T cell"], 'T cells', inplace=True)
